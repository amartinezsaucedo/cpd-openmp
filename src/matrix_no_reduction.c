/******************************************************
    ...
    Objetivo:   Desarrollar una versión paralela de la aplicación para multiplicar
                matrices utilizando OpenMP con cualquier directiva excepto “omp reduction”.
...
struct matrix *multiply_matrices(const struct matrix *matrix_a, const struct matrix *matrix_b)
{
    ...
    // través de las filas y columnas). Para evitar condiciones de carrera entre los threads al calcular result, se estableció
    // como variable privada a result además de los distintos índices, ya que la utilización de mecanismos de sincronización 
    // dentro del ciclo que calcula el valor de result no ha generado los resultados esperados en términos de cálculo y 
