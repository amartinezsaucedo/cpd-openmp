/******************************************************
    ...
    Objetivo:   Desarrollar una versión paralela de la aplicación para multiplicar
                matrices utilizando OpenMP y “omp parallel for” en conjunto con “omp reduction”.
...
struct matrix* multiply_matrices(const struct matrix* matrix_a, const struct matrix* matrix_b)
{
	...
	// para una posición [i, j] de la matriz resultado.
    #pragma omp parallel for private(i, j, k, l) reduction(+: result) collapse(2) 
